...
class OfflineModel:
    ...
    #         models_l = []
    #         for tau in quantile_grid:
    #             model = QuantileRegressionNN(input_dim=X.shape[1], hidden_dim=hidden_dim).to(device)
    #             optimizer = optim.Adam(model.parameters(), lr=lr)
...
def plot_theta_distribution_offline(x, y,x_new):
    ...
    plt.savefig('distribution1.pdf', format='pdf')
    plt.show()
    ...
    if d == 2:
        ...
        plt.savefig('Scatter1.pdf', format='pdf')
        plt.tight_layout()
        plt.show()
...
if __name__ == '__main__':
    ...
    plt.grid(True)
    plt.savefig('qrgmm1.pdf', format='pdf')
    plt.show()
